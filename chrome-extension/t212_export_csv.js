...
function holdingsToCSV() {
  ...
  let getRowData = row => {
    ...
    return cells.map((c, i) => {
      ...
      // Quantity
      if (isData && i == 2) {
        return parseFloat(c.innerText.replace(/\s/g, '')).toFixed(4)
      }

      // Price
      if (isData && i == 3) {
        return parseFloat(c.innerText.replace(/\s/g, '')).toFixed(2)
      }
